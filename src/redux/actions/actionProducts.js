...
export const editAsync = (nombre, producto) => {
    ...
    return async (dispatch) => {
        const collectionTraer = collection(db, "DbProducts");
        ...
        traerDatosQ.forEach( async (docu) => {
          ...
          console.log(id);
        const documRef = doc(db, "DbProducts", id)
...
export const deleteAsync = (nombre) => {
    return async (dispatch) => {
        ...
        traerDatosQ.forEach((docum => {
            deleteDoc(doc(db, "DbProducts", docum.id))
        }))
...
export const listAsync = () => {
    return async (dispatch) => {
        const collectionTraer = await getDocs(collection(db, "DbProducts"));
        const productos = [];
...
export const addAsync = (producto) => {
    return (dispatch) => {
        addDoc(collection(db, "DbProducts"), producto)
            .then(resp => {
